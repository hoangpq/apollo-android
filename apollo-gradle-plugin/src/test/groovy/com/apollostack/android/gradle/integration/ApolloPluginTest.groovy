...
class ApolloPluginTest extends Specification {
  ...
  def "testProject builds successfully and generates expected outputs"() {
    ...
    assert new File(testProjectDir, "src/release/graphql/generatedIR/ReleaseAPI.json").isFile()
    assert new File(testProjectDir, "src/debug/graphql/generatedIR/DebugAPI.json").isFile()

    assert new File(testProjectDir, "build/generated/source/apollo/generatedIR/DroidDetails.java").isFile()
    assert new File(testProjectDir, "build/generated/source/apollo/generatedIR/Films.java").isFile()
  }
  ...
  def "generateApolloIR generates IR files successfully"() {
    setup: "a testProject with no previously generated IRs"
    prepareTestProjectDir()
    removeIRFiles()
    ...
    def result = GradleRunner.create().withProjectDir(new File(testProjectDir))
        .withPluginClasspath()
        .withArguments("generateApolloIR")
        .forwardStdError(new OutputStreamWriter(System.err)).build()
  ...
  private static def removeIRFiles() {
    new File(testProjectDir, "src").eachFileRecurse { file ->
      if (file.name.endsWith('API.json')) {
        file.delete()
      }
    }
  }
